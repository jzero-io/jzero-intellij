...
public class TypeGotoDeclarationHandler implements LineMarkerProvider {
    ...
    public LineMarkerInfo<?> getLineMarkerInfo(@NotNull PsiElement element) {
        ...
        // Skip the "type" keyword to avoid duplicate markers
        String elementText = element.getText();
        if ("type".equals(elementText)) {
            return null;
        }

        // Show icons for type names (identifiers) in type definitions
        // Check if element text could be a type name (starts with uppercase letter)
        if (elementText == null || elementText.length() == 0 || !Character.isUpperCase(elementText.charAt(0))) {
            return null;
        }

        // Check if this element is part of a type definition
        String typeName = extractTypeNameFromContext(element);
        if (typeName == null) {
            return null;
        }

        // Additional check: ensure the element is actually the type name in the line
    ...
    /**
     * Check if this element is the first occurrence of the type name in the line
     */
    private boolean isFirstOccurrenceInLine(@NotNull PsiElement element, @NotNull String typeName) {
        String lineText = getLineText(element);
        if (lineText == null) {
            return true;
        }

        // Find the first occurrence of the type name in the line
        int firstIndex = lineText.indexOf(typeName);

        // Get the offset of our element within the line
        String content = element.getContainingFile().getText();
        int elementOffset = element.getTextOffset();

        // Find the start of the line containing our element
        int lineStart = elementOffset;
        while (lineStart > 0 && content.charAt(lineStart - 1) != '\n') {
            lineStart--;
        }

        int ourIndex = elementOffset - lineStart;

        // Return true only if our element is the first occurrence
        return ourIndex == firstIndex;
    }
    ...
    @Nullable
    private String extractTypeNameFromContext(@NotNull PsiElement element) {
        String lineText = getLineText(element);
        if (lineText == null) {
            return null;
        }
        ...
        // Must contain braces to be a type definition
        if (!trimmedLine.contains("{")) {
            return null;
        }

        int braceIndex = trimmedLine.indexOf('{');
        String beforeBrace = trimmedLine.substring(0, braceIndex).trim();

        // Check if this line is a type definition line
        // Format 1: type TypeName {
        // Format 2: TypeName { (inside type () block)

        // Find all words before the brace
        String[] words = beforeBrace.split("\\s+");
        if (words.length == 0) {
            return null;
        }

        // The type name is the last word before "{"
        String typeName = words[words.length - 1];

        // Check if the element text matches the type name
        if (!typeName.equals(elementText)) {
            return null;
        }

        // Validate formats:
        // Format 1: "type TypeName {" - should start with "type"
        // Format 2: "TypeName {" - should be inside a type () block
        if (words.length == 2 && "type".equals(words[0])) {
            // Format 1: type TypeName {
            return typeName;
        } else if (words.length == 1) {
            // Format 2: TypeName { - check if inside type () block
            if (isInsideTypeBlock(element)) {
                return typeName;
            }
        }
    ...
    /**
     * Check if the element is inside a type () block
     */
    private boolean isInsideTypeBlock(@NotNull PsiElement element) {
        ...
        // Search backwards from element to find "type (" pattern
        int offset = elementOffset;
        int typeOpenParen = -1;
        int typeCloseParen = -1;

        while (offset > 0) {
            offset--;
            char c = content.charAt(offset);
            if (c == ')') {
                typeCloseParen = offset;
            } else if (c == '(' && typeCloseParen == -1) {
                ...
                String lineBefore = content.substring(lineStart, offset).trim();
                if (lineBefore.equals("type")) {
                    typeOpenParen = offset;
                    break;
                }
            }
        }

        // If we found type ( and haven't found closing ), we're inside the block
        return typeOpenParen != -1 && typeCloseParen == -1;
    }
